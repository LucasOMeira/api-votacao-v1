...
public class EleitorController {
    ...
    public ResponseEntity<VotoDTO> votar(@PathVariable Long id, @RequestBody VotoDTO votoDTO) {
        try {
    ...
    public ResponseEntity<Eleitor> salvarEleitor(@RequestBody Eleitor eleitor) {
        ...
        return ResponseEntity.ok(savedEleitor);
    }
    ...
    public ResponseEntity<Void> deletarEleitor(@PathVariable Long id) {
        try {
            ...
            return ResponseEntity.notFound().build(); // Retorna 404 Not Found se o eleitor não for encontrado
        }
    }
}
